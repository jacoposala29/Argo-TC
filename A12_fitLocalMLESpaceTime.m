...
H = load([hprefix,num2str(windowSize), '_', num2str(minNumberOfObs),'_',var2use,'.mat']);
B = load([folder2use '/Outputs/localMLESpaceTime_Depth_',...
...
%% Save results
save([folder2use '/Outputs/localMLESpaceTimeCoefs_Depth_',num2str(current_layer,'%03d'),...
    '_',num2str(windowSize), '_',...
    num2str(minNumberOfObs),'_',num2str(windowSizeGP), '_',...
    num2str(month,'%02d'),'_',num2str(startYear),'_', num2str(endYear),...
    region,'.mat'],...
'FloatIDReg', 'CycleNumberReg', 'interpLat', 'interpLong', 'sigma',...
'thetaLat', 'thetaLong', 'thetas', 'thetat', 'nll', 'nResGrid',...
'interpLat', 'interpLong', '-v7.3');
