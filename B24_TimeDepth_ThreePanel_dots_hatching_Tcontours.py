...
for sub, sub_comp in zip(sub_list, sub_list_comparison):
    ...
    if sub_comp:   
        ...
        # plt.pcolor(data_xaxis, data_yaxis, msk_data, hatch='.', alpha=0.)
        nlevels=5
        # levels_T_positive = np.linspace(0, np.nanmax(msk_data), num=nlevels)
        # levels_T_negative = np.linspace(np.nanmin(msk_data), 0, num=nlevels)
        min_value = np.nanmin(msk_data)
        max_value = np.nanmax(msk_data)
        levels_T_positive = np.arange(0, max_value + 0.2, 0.2)
        levels_T_negative = np.arange(min_value, 0, 0.2)
        
        contour_T_pos = ax1.contour(data_xaxis, data_yaxis, msk_data,
                           levels=levels_T_positive, colors='black', linestyles='solid')
        contour_T_neg = ax1.contour(data_xaxis, data_yaxis, msk_data,
                           levels=levels_T_negative, colors='black', linestyles='dashed')
        ...
        plt.savefig('figure_every02.png', transparent=True)
