...
class Processor:
    def __init__(self, hurricane_df, f):
        ...
        self.cycle_nums = np.array(f['profCycleNumberAggrSel']).flatten()
        self.float_ids = np.array(f['profFloatIDAggrSel']).flatten()
        self.argo_dates = np.array(f['profJulDayAggrSel']).flatten()
        self.argo_datetimes = matlab_to_datetime_v(self.argo_dates)
        self.argo_lons = conform_lons(
                np.array(f['profLongAggrSel'])).flatten()
        self.argo_lats = np.array(f['profLatAggrSel']).flatten()
    ...
    def create_pair_df(self):
        ...
        for idx, row in df.iterrows():
            ...
            df_lst.append(pd.DataFrame({
                ...
                'argo_lat':     [row['argo_lat'] for _ in range(n)],
                'argo_lon':     [row['argo_lon'] for _ in range(n)],
                }))
