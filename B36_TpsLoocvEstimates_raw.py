...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 24 15:54:02 2021
...
from TC_and_TS_define_param import (grid_lower, grid_upper, grid_stride, folder2use, var2use, 
...
for sub in sub_list:
    ...
    # if lamb_choice == 'AdaptInflate': # Choice of the smallest lambda with a score = 1% more than the minimum score
    #     min_error = error_estimates.min(axis=0)
    #     inflation_factor = 1.01
    #     lhat_inflated_all = np.argmax(error_estimates < min_error * inflation_factor, axis=0)
    #     lamb_seq = LAMB[lhat_inflated_all]
...
# OLD METHODS TO SELECT LAMBDA
    ...
    pkl.dump(PREDS_NOREW,
            open(f'{output_dir}/TPS_LOOCV_Preds_NoRew_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    pkl.dump(PREDS_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_Preds_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    pkl.dump(STDEV_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_Stdev_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    pkl.dump(MASK_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_Mask_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    
    pkl.dump(THETA_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_Theta_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    pkl.dump(BASIS_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_Basis_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
    pkl.dump(COV_THETA_BLOCK,
            open(f'{output_dir}/TPS_LOOCV_CovTheta_Block_{lamb_choice}_{sub}{stagetag}_{degrees_tag}.pkl', 'wb'))
